...
/**
 * Servlet that generates JPEG images representing a designated message with an
 * oblique shadowed version behind it.
 * <p>
...
public class ShadowedText extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		if (wantsList != null) {	// Nếu lựa chọn show list font
			showFontList(response);
		} else {
			// lấy trường message và kiểm tra dữ liệu nhập vào
			String message = request.getParameter("message");
			if ((message == null) || (message.length() == 0)) {
				message = "Missing 'message' parameter";
			}
			
			// lấy trường fontName và kiểm tra dữ liệu nhập vào
			String fontName = request.getParameter("fontName");
			if ((fontName == null) || (fontName.length() == 0)) {
				fontName = "Serif";
			}
			
			// lấy trường fontSizeString và kiểm tra dữ liệu nhập vào
			...
			// thiết lập loại nội dung, trả về 1 hình ảnh
			response.setContentType("image/jpeg");
			// gọi phương thức để vẽ ảnh
